    ...
    */
    jQuery.event = {
        ...
        */
        ...
        /**
         分派(执行)事件处理函数
         * @param {Object} 原生Event事件对象
         * @return {...}  返回事件处理函数结果
         */
        dispatch: function( event ) {

            // Make a writable jQuery.Event from the native event object
            // 使用fix修正Event，生成一个可写的事件对象,返回实例化的jQuery.Event对象
            ...
            while ( (matched = handlerQueue[ i++ ]) && !event.isPropagationStopped() ) {
                ...
                while ( (handleObj = matched.handlers[ j++ ]) && !event.isImmediatePropagationStopped() ) {
                    ...
                    if ( !event.namespace_re || event.namespace_re.test( handleObj.namespace ) ) {
                        ...
                        event.handleObj = handleObj;   //事件处理器对象        dxi
                        event.data = handleObj.data;   // 自定义的参数
        ...
        /**
         修正原生Event事件对象，生成新的封装的jQuery.Event事件对象,代替原生的Event对象
         * @param {Object} event 事件对象
         * @return {Object} 返回封装的jQuery.Event事件对象
         */
        fix: function( event ) {
            // 重写原生事件对象，变成一个可读写的对象，方便未来修改、扩展
            // 检测已经fixed（修正）过，表明是jQuery.Event
    ...
    /**
     jQuery为统一原生Event对象而封装的jQuery.Event类
     * @param {Object} src 原生Event事件对象
     * @param {Object Or Undefined} props (可选) 自定义的属性,将被扩展到实例化的jQuery.Event对象中
     */
    jQuery.Event = function( src, props ) {
        ...
        if ( src && src.type ) {
            ...
            // 扩充了DOM3 事件方法
            // isDefaultPrevented() 检查是否阻止了默认动作，阻止返回true，没有返回false
            // 标准的DOM事件defaultPrevented 检测是否
            // returnValue默认为true 设置false就可以取消事件的默认行为(读/写)
        ...
        if ( props ) {
            // 存在则自定义的props(属性)将被extend(扩展)进创建的event中作为属性
...
// http://www.w3.org/TR/2003/WD-DOM-Level-3-Events-20030331/ecma-script-binding.html
    jQuery.Event.prototype = {
        isDefaultPrevented: returnFalse,        // 返回指定的 Event对象上是否调用了 preventDefault() 方法
        isPropagationStopped: returnFalse,     // 返回指定的 Event对象上是否调用了 stopPropagation() 方法
        isImmediatePropagationStopped: returnFalse,    // 返回指定的 Event对象是否调用了 stopImmediatePropagation() 方法
...
// because jsdom on node.js will break without it.
    ...
    jQuery.extend({
        ...
        css: function( elem, name, extra, styles ) {
            var val, num, hooks,
                //转成驼峰形式，background-color,backgroundColor"
                origName = jQuery.camelCase( name );
